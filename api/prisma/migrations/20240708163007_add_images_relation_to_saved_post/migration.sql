/*
  ...
  - Added the required column `imagesId` to the `SavedPost` table without a default value. This is not possible if the table is not empty.
...
CREATE TABLE "new_SavedPost" (
    ...
    CONSTRAINT "SavedPost_imagesId_fkey" FOREIGN KEY ("imagesId") REFERENCES "Image" ("id") ON DELETE RESTRICT ON UPDATE CASCADE
);
...
CREATE UNIQUE INDEX "SavedPost_userId_postId_imagesId_key" ON "SavedPost"("userId", "postId", "imagesId");
